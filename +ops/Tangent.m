classdef Tangent < core.GeomFeature
    ...
    methods
        function obj = Tangent(varargin)
            [ctx, target, args] = ops.Tangent.parse_inputs(varargin{:});
            ...
            if ~isempty(args.edge2)
                if ~isa(args.edge2, 'core.GeomFeature')
                    error("Tangent edge2 must be a core.GeomFeature.");
                end
                obj.add_input(args.edge2);
            end
            if ~isempty(args.point_target)
                if ~isa(args.point_target, 'core.GeomFeature')
                    error("Tangent point_target must be a core.GeomFeature.");
                end
        ...
        function set.type(obj, val)
            t = ops.Tangent.normalize_type(val);
        ...
        function val = get.coord(obj)
            val = obj.get_param("coord", types.Vertices([1, 0]));
        end
        ...
        function val = get.start(obj)
            val = obj.get_param("start", types.Parameter(0.5, "", unit=""));
        end
        ...
        function val = get.start2(obj)
            val = obj.get_param("start2", types.Parameter(0.5, "", unit=""));
        end

        function set.edge2(obj, val)
            if ~isempty(val) && ~isa(val, 'core.GeomFeature')
                error("Tangent edge2 must be empty or a core.GeomFeature.");
            end
        ...
        function set.point_target(obj, val)
            if ~isempty(val) && ~isa(val, 'core.GeomFeature')
                error("Tangent point_target must be empty or a core.GeomFeature.");
            end
        ...
        function set.edge_index(obj, val)
            ...
            obj.set_param("edge_index", types.Parameter(n, "", unit=""));
        end

        function val = get.edge_index(obj)
            val = obj.get_param("edge_index", types.Parameter(1, "", unit=""));
        end
        ...
        function set.edge2_index(obj, val)
            ...
            obj.set_param("edge2_index", types.Parameter(n, "", unit=""));
        end

        function val = get.edge2_index(obj)
            val = obj.get_param("edge2_index", types.Parameter(1, "", unit=""));
        end
        ...
        function set.point_index(obj, val)
            ...
            obj.set_param("point_index", types.Parameter(n, "", unit=""));
        end

        function val = get.point_index(obj)
            val = obj.get_param("point_index", types.Parameter(1, "", unit=""));
        end
    ...
    methods (Static, Access=private)
        function [ctx, target, args] = parse_inputs(varargin)
            [ctx, target, nv] = core.GeomFeature.parse_target_context("Tangent", varargin{:});
            args = ops.Tangent.parse_options(nv{:});
        end
