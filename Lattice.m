classdef Lattice < handle
    ...
    methods
        ...
        function lattice_nodes = computeGeometry(obj, name)
            ...
            canonical_name = Lattice.canonical_lattice_name(name);
            ...
            if isstruct(obj.nodes)
                switch key
                    case {"a", "sitea", "sublatticea"}
                        sites = Lattice.extract_site_field(obj.nodes, "sublatticeA", args.as_grid);
                    case {"b", "siteb", "sublatticeb"}
                        sites = Lattice.extract_site_field(obj.nodes, "sublatticeB", args.as_grid);
                    case {"center", "centers", "hexagon_center", "hexagon_centers"}
                        sites = Lattice.extract_site_field(obj.nodes, "hexagon_centers", args.as_grid);
                    case {"all", "*"}
                        sites = struct();
                        sites.A = Lattice.extract_site_field(obj.nodes, "sublatticeA", args.as_grid);
                        sites.B = Lattice.extract_site_field(obj.nodes, "sublatticeB", args.as_grid);
                        sites.centers = Lattice.extract_site_field(obj.nodes, "hexagon_centers", args.as_grid);
                    otherwise
                ...
                sites = Lattice.flatten_nodes(obj.nodes, args.as_grid);
            end
        end
    ...
    methods (Static)
        function lattice = from_name(name, a, nw, nh)
            ...
            switch canonical_name
                case "Hexagonal"
                    lattice = HexagonalLattice(a, nw, nh);
                case "Honeycomb"
                    lattice = HoneyCombLattice(a, nw, nh);
                otherwise
                    error("Unsupported lattice type '%s'.", char(string(name)));
            end
        end

        function [feature, lattice] = createArray(args)
            % Create a lattice object and replicate seed feature(s) at its sites.
            %
            % Hexagonal usage:
            %   [f, lat] = Lattice.createArray(lattice="Hexagonal", a=..., nw=..., nh=..., ...
            %       seed=myFeature, ctx=ctx, a_parameter=a_param, layer="metal1");
            %
            % Honeycomb usage with two site types:
            %   [f, lat] = Lattice.createArray(lattice="HoneyComb", a=..., nw=..., nh=..., ...
            ...
            lattice = Lattice.from_name(args.lattice, args.a, args.nw, args.nh);
            if isa(lattice, "HexagonalLattice")
                if isempty(args.seed)
                    error("Lattice.createArray for Hexagonal lattice requires 'seed'.");
                end
                feature = lattice.arrayFromFeature(seed=args.seed, ctx=args.ctx, ...
                    layer=args.layer, a_parameter=args.a_parameter, offset=args.offset);
                return;
            end

            if isa(lattice, "HoneyCombLattice")
                if isempty(args.seed) && isempty(args.seedA) && isempty(args.seedB) && isempty(args.seedCenters)
                    error("Lattice.createArray for HoneyComb lattice requires seed or seedA/seedB/seedCenters.");
                end
                feature = lattice.arrayFromFeature(ctx=args.ctx, seed=args.seed, ...
            ...
            error("Lattice.createArray does not support class '%s'.", class(lattice));
        end
    end
    ...
    methods (Access=private, Static)
        ...
        function out = extract_site_field(nodes_struct, field_name, as_grid)
            ...
            out = Lattice.flatten_nodes(nodes_struct.(field_name), as_grid);
        end
