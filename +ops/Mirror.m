classdef Mirror < core.GeomFeature
    ...
    methods
        function obj = Mirror(varargin)
            [ctx, target, args] = ops.Mirror.parse_inputs(varargin{:});
        ...
        function set.point(obj, val)
            obj.set_param("point", core.GeomFeature.coerce_vertices(val));
        end
        ...
        function set.axis(obj, val)
            obj.set_param("axis", core.GeomFeature.coerce_vertices(val));
        end
    ...
    methods (Static, Access=private)
        function [ctx, target, args] = parse_inputs(varargin)
            [ctx, target, nv] = core.GeomFeature.parse_target_context("Mirror", varargin{:});
            args = ops.Mirror.parse_options(nv{:});
        end
