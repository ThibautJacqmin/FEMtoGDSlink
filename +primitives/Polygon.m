classdef Polygon < core.GeomFeature
    ...
    properties (Access=private)
        vertices_ types.Vertices = types.Vertices.empty
    end
    methods
        function obj = Polygon(varargin)
            obj@core.GeomFeature();
            if nargin == 0
                obj.inputs = {};
                obj.params = struct();
            ...
            [ctx, args] = primitives.Polygon.parse_inputs(varargin{:});
            if isempty(ctx)
        ...
        function set.vertices(obj, val)
            if isempty(val)
                obj.vertices_ = types.Vertices.empty;
                return;
            end
