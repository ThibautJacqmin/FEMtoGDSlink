classdef Tangent < GeomFeature
    ...
    methods
        function obj = Tangent(varargin)
            ...
            if ~isempty(args.edge2)
                if ~isa(args.edge2, 'GeomFeature')
                    error("Tangent edge2 must be a GeomFeature.");
                end
                obj.add_input(args.edge2);
            end
            if ~isempty(args.point_target)
                if ~isa(args.point_target, 'GeomFeature')
                    error("Tangent point_target must be a GeomFeature.");
                end
        ...
        function val = get.coord(obj)
            val = obj.get_param("coord", Vertices([1, 0]));
        end

        function set.start(obj, val)
            p = GeomFeature.coerce_parameter(val, "", unit="");
        ...
        function val = get.start(obj)
            val = obj.get_param("start", Parameter(0.5, "", unit=""));
        end
        ...
        function val = get.start2(obj)
            val = obj.get_param("start2", Parameter(0.5, "", unit=""));
        end

        function set.edge2(obj, val)
            if ~isempty(val) && ~isa(val, 'GeomFeature')
                error("Tangent edge2 must be empty or a GeomFeature.");
            end
        ...
        function set.point_target(obj, val)
            if ~isempty(val) && ~isa(val, 'GeomFeature')
                error("Tangent point_target must be empty or a GeomFeature.");
            end
        ...
        function set.edge_index(obj, val)
            ...
            obj.set_param("edge_index", Parameter(n, "", unit=""));
        end

        function val = get.edge_index(obj)
            val = obj.get_param("edge_index", Parameter(1, "", unit=""));
        end
        ...
        function set.edge2_index(obj, val)
            ...
            obj.set_param("edge2_index", Parameter(n, "", unit=""));
        end

        function val = get.edge2_index(obj)
            val = obj.get_param("edge2_index", Parameter(1, "", unit=""));
        end
        ...
        function set.point_index(obj, val)
            ...
            obj.set_param("point_index", Parameter(n, "", unit=""));
        end

        function val = get.point_index(obj)
            val = obj.get_param("point_index", Parameter(1, "", unit=""));
        end
    ...
    methods (Static, Access=private)
        function [ctx, target, args] = parse_inputs(varargin)
            [ctx, target, nv] = GeomFeature.parse_target_context("Tangent", varargin{:});
