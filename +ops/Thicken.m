classdef Thicken < core.GeomFeature
    ...
    methods
        function obj = Thicken(varargin)
            [ctx, target, args] = ops.Thicken.parse_inputs(varargin{:});
        ...
        function set.offset(obj, val)
            obj.set_param("offset", ops.Thicken.normalize_offset(val));
        end
        ...
        function set.totalthick(obj, val)
            p = core.GeomFeature.coerce_parameter(val, "totalthick");
        ...
        function val = get.totalthick(obj)
            val = obj.get_param("totalthick", types.Parameter(1, "", unit="nm"));
        end

        function set.upthick(obj, val)
            p = core.GeomFeature.coerce_parameter(val, "upthick");
            if ~(isscalar(p.value) && isfinite(p.value) && p.value >= 0)
        ...
        function val = get.upthick(obj)
            val = obj.get_param("upthick", types.Parameter(0.5, "", unit="nm"));
        end

        function set.downthick(obj, val)
            p = core.GeomFeature.coerce_parameter(val, "downthick");
            if ~(isscalar(p.value) && isfinite(p.value) && p.value >= 0)
        ...
        function val = get.downthick(obj)
            val = obj.get_param("downthick", types.Parameter(0.5, "", unit="nm"));
        end

        function set.ends(obj, val)
            obj.set_param("ends", ops.Thicken.normalize_ends(val));
        end
        ...
        function set.convexcorner(obj, val)
            obj.set_param("convexcorner", ops.Thicken.normalize_convexcorner(val));
        end
    ...
    methods (Static, Access=private)
        function [ctx, target, args] = parse_inputs(varargin)
            [ctx, target, nv] = core.GeomFeature.parse_target_context("Thicken", varargin{:});
            args = ops.Thicken.parse_options(nv{:});
        end
