classdef Polygon < Klayout
    ...
    methods
        ...
        function y = make_2D_array(obj, ncopies_x, ncopies_y, vertices, gds_modeler, layer)
            ...
            y = Polygon;
            if ~isempty(obj.klayout_adapter)
                array_adapter = obj.klayout_adapter.make2DArray(obj, ncopies_x, ncopies_y, vertices, gds_modeler, layer);
                if ~isempty(array_adapter)
                    y.assign_klayout_adapter(array_adapter);
                else
                    region = obj.make_2D_array_fallback(ncopies_x, ncopies_y, vertices, gds_modeler, layer);
                    y.pgon_py = region;
                end
            else
                region = obj.make_2D_array_fallback(ncopies_x, ncopies_y, vertices, gds_modeler, layer);
        ...
        function y = make_1D_array(obj, ncopies, vertex, gds_modeler, layer)
            ...
            y = Polygon;
            if ~isempty(obj.klayout_adapter)
                array_adapter = obj.klayout_adapter.make1DArray(obj, ncopies, vertex, gds_modeler, layer);
                if ~isempty(array_adapter)
                    y.assign_klayout_adapter(array_adapter);
                else
                    region = obj.make_1D_array_fallback(ncopies, vertex, gds_modeler, layer);
                    y.pgon_py = region;
                end
            else
                region = obj.make_1D_array_fallback(ncopies, vertex, gds_modeler, layer);
        ...
        function adopt_klayout_shape(obj, shape_handle)
            if nargin < 2 || isempty(shape_handle)
                return;
            end
            if isempty(obj.klayout_adapter)
                obj.klayout_adapter = KLayoutShapeAdapter(obj.pya);
            end
            obj.klayout_adapter.replaceShape(shape_handle);
            obj.sync_klayout_shape();
        end
    ...
    methods (Access = private)
        function region = make_1D_array_fallback(obj, ncopies, vertex, gds_modeler, layer)
        ...
        function region = make_2D_array_fallback(obj, ncopies_x, ncopies_y, vertices, gds_modeler, layer)
