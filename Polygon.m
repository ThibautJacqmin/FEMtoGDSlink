classdef Polygon < GeomFeature
    % Polygon primitive represented by an ordered list of 2D vertices.
    %
    % The class has two modes:
    % 1) Feature mode (with a GeometrySession): registers as a graph node.
    ...
    properties
        pgon_py = []
        comsol_modeler = ComsolModeler.empty
        comsol_shape = []
        comsol_name = ""
    end
    properties (Access=private)
        vertices_ Vertices = Vertices.empty
    end
    methods
    ...
    methods (Static, Access=private)
        function [ctx, args] = parse_inputs(varargin)
            if ~isempty(varargin) && isa(varargin{1}, 'GeometrySession')
                ctx = varargin{1};
                nv = varargin(2:end);
            else
