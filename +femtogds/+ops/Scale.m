classdef Scale < femtogds.core.GeomFeature
    ...
    methods
        function obj = Scale(varargin)
            [ctx, target, args] = femtogds.ops.Scale.parse_inputs(varargin{:});
        ...
        function set.factor(obj, val)
            obj.set_param("factor", femtogds.core.GeomFeature.coerce_parameter(val, "factor", unit=""));
        end
        ...
        function set.origin(obj, val)
            obj.set_param("origin", femtogds.core.GeomFeature.coerce_vertices(val));
        end
    ...
    methods (Static, Access=private)
        function [ctx, target, args] = parse_inputs(varargin)
            [ctx, target, nv] = femtogds.core.GeomFeature.parse_target_context("Scale", varargin{:});
            args = femtogds.ops.Scale.parse_options(nv{:});
        end
