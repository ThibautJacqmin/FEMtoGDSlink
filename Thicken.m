classdef Thicken < GeomFeature
    ...
    methods
        ...
        function set.totalthick(obj, val)
            p = GeomFeature.coerce_parameter(val, "totalthick");
        ...
        function val = get.totalthick(obj)
            val = obj.get_param("totalthick", Parameter(1, "", unit="nm"));
        end

        function set.upthick(obj, val)
            p = GeomFeature.coerce_parameter(val, "upthick");
            if ~(isscalar(p.value) && isfinite(p.value) && p.value >= 0)
        ...
        function val = get.upthick(obj)
            val = obj.get_param("upthick", Parameter(0.5, "", unit="nm"));
        end

        function set.downthick(obj, val)
            p = GeomFeature.coerce_parameter(val, "downthick");
            if ~(isscalar(p.value) && isfinite(p.value) && p.value >= 0)
        ...
        function val = get.downthick(obj)
            val = obj.get_param("downthick", Parameter(0.5, "", unit="nm"));
        end
    ...
    methods (Static, Access=private)
        function [ctx, target, args] = parse_inputs(varargin)
            [ctx, target, nv] = GeomFeature.parse_target_context("Thicken", varargin{:});
