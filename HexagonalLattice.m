classdef HexagonalLattice < Lattice
    ...
    methods
        ...
        function feature = circleArray(obj, args)
            % Build a circle copied at each hexagonal lattice site using Array features.
            arguments
                obj
            ...
            feature = obj.arrayFromFeature(seed=seed, ctx=ctx, layer=args.layer, ...
                a_parameter=a_param, offset=args.offset);
        end
        ...
        function feature = arrayFromFeature(obj, args)
            ...
            arguments
                obj
                args.seed femtogds.core.GeomFeature
                args.ctx femtogds.core.GeometrySession = femtogds.core.GeometrySession.empty
                args.layer = []
                args.a_parameter = []
                args.offset = [0, 0] % coefficient in units of lattice constant
            ...
            nx = obj.nw + 1;
            delta_x = femtogds.types.Vertices([1, 0], a_param);
            delta_y = femtogds.types.Vertices([0, sqrt(3)], a_param);
            ...
            for i = 1:numel(families)
                ...
                arr = femtogds.ops.Array2D(ctx, args.seed, ...
                    ncopies_x=nx, ncopies_y=fam.ny, ...
                    delta_x=delta_x, delta_y=delta_y, ...
                    layer=layer);
                if any(abs(shift) > 1e-12)
                    arr = femtogds.ops.Move(ctx, arr, ...
                        delta=femtogds.types.Vertices(shift, a_param), layer=layer);
                end
            ...
            elseif isscalar(arrays)
                ...
                feature = femtogds.ops.Union(ctx, arrays, layer=layer);
            end
        end
    end
    methods (Access=private)
        function p = coerce_lattice_parameter(obj, a_input)
            if isempty(a_input)
                p = femtogds.types.Parameter(obj.a, "", auto_register=false);
            elseif isa(a_input, "femtogds.types.Parameter")
                p = a_input;
            else
                p = femtogds.types.Parameter(a_input, "", auto_register=false);
            end
