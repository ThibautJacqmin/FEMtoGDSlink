classdef GeomFeature < handle
    ...
    methods
        function obj = GeomFeature(ctx, layer)
            % Construct a graph node bound to a femtogds.core.GeometrySession context.
            if nargin < 1
                return;
            end
        ...
        function ctx = context(obj)
            % Return owning femtogds.core.GeometrySession.
            ctx = obj.ctx;
        end
    ...
    methods (Static, Access=protected)
        function p = coerce_parameter(val, default_name, args)
            % Coerce scalar or femtogds.types.Parameter to femtogds.types.Parameter with optional unit override.
            arguments
                val
            ...
            p = femtogds.types.Parameter(val, "", unit=args.unit);
        end

        function v = coerce_vertices(val)
            % Coerce numeric 2D value or femtogds.types.Vertices to femtogds.types.Vertices instance.
            if isa(val, 'femtogds.types.Vertices')
                v = val;
                return;
            end
            v = femtogds.types.Vertices(val);
        end
        ...
        function [ctx, target, nv] = parse_target_context(op_name, varargin)
            ...
            if isa(varargin{1}, 'femtogds.core.GeometrySession')
                ctx = varargin{1};
                if numel(varargin) < 2
                    error("%s requires a target feature.", char(op));
                end
                ...
                target = varargin{1};
                if isa(target, 'femtogds.core.GeomFeature')
                    ctx = target.context();
                else
                    ctx = femtogds.core.GeometrySession.require_current();
                end
        ...
        function [ctx, members, nv] = parse_members_context(op_name, varargin)
            ...
            if isa(varargin{1}, 'femtogds.core.GeometrySession')
                ...
                members = varargin{1};
                members = femtogds.core.GeomFeature.normalize_feature_inputs(members, op);
                if ~isempty(members)
        ...
        function [ctx, base, tools, nv] = parse_base_tools_context(op_name, varargin)
            ...
            if isa(varargin{1}, 'femtogds.core.GeometrySession')
                ...
                base = varargin{1};
                tools = varargin{2};
                if isa(base, 'femtogds.core.GeomFeature')
                    ctx = base.context();
                else
                    ctx = femtogds.core.GeometrySession.require_current();
                end
            ...
            tools = femtogds.core.GeomFeature.normalize_feature_inputs(tools, op + " tool");
        end
        ...
        function inputs = normalize_feature_inputs(members, op_name)
            ...
            for i = 1:numel(inputs)
                if ~isa(inputs{i}, 'femtogds.core.GeomFeature')
