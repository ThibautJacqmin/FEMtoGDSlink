classdef Parameter
    ...
    methods
        function obj = Parameter(primary, secondary, tertiary, args)
            ...
            expr_override = types.Parameter.resolve_expr_override(args.expr, args.expression);
            dep_records = struct('name', {}, 'value', {}, 'unit', {}, 'expr', {});
            name_kw = types.Parameter.normalize_name(args.name);

            if isa(primary, "function_handle")
                deps = types.Parameter.normalize_dependencies(secondary);
                name_pos = types.Parameter.normalize_name(tertiary, ...
                    "Function-based Parameter requires a name as third positional argument or name=""..."".");
                name = types.Parameter.resolve_name_inputs(name_pos, name_kw);
                if strlength(name) == 0
                ...
                [value, inferred_expr, dep_records, inferred_unit] = ...
                    types.Parameter.build_from_function(primary, deps);
                expr = inferred_expr;
                unit = types.Parameter.resolve_unit(args.unit, inferred_unit);
            elseif isa(primary, "types.Parameter")
                ...
                source = primary;
                value = source.value;
                name_pos = types.Parameter.normalize_name(secondary);
                name = types.Parameter.resolve_name_inputs(name_pos, name_kw);
                expr = source.expression_token();
                unit = types.Parameter.resolve_unit(args.unit, source.unit);
            ...
            elseif isnumeric(primary) && isscalar(primary)
                ...
                expr = "";
                unit = types.Parameter.resolve_unit(args.unit, "nm");
            else
                error(['Parameter primary input must be scalar numeric, types.Parameter, ' ...
                    'or function handle.']);
            end
            ...
            if obj.is_named()
                self_rec = struct( ...
                    ...
                    'unit', obj.unit, ...
                    'expr', obj.expr);
                obj.dependency_records = types.Parameter.merge_dependency_records( ...
                    obj.dependency_records, self_rec);
            end

            if args.auto_register
                types.Parameter.try_register_current_context(obj);
            end
        end
        ...
        function y = sin(~)
            ...
            y = types.Parameter.empty;
            types.Parameter.unsupported_function_error("sin");
        end

        function y = cos(~)
            % Disallow direct symbolic math on Parameter; use function-based constructor.
            y = types.Parameter.empty;
            types.Parameter.unsupported_function_error("cos");
        end

        function y = exp(~)
            % Disallow direct symbolic math on Parameter; use function-based constructor.
            y = types.Parameter.empty;
            types.Parameter.unsupported_function_error("exp");
        end
    end
    methods (Static, Access=private)
        ...
        function y = apply_binary(lhs, rhs, operation)
            ...
            [lhs_value, lhs_expr, lhs_unit, lhs_records] = types.Parameter.coerce_operand(lhs);
            [rhs_value, rhs_expr, rhs_unit, rhs_records] = types.Parameter.coerce_operand(rhs);
            ...
            y_unit = types.Parameter.combine_units(lhs_unit, rhs_unit, operation);
            y = types.Parameter(y_value, "", unit=y_unit, expr=y_expr);
            y.dependency_records = types.Parameter.merge_dependency_records(lhs_records, rhs_records);
        end
        ...
        function deps = normalize_dependencies(dep_input)
            ...
            if isa(dep_input, "types.Parameter")
            ...
            for i = 1:numel(deps)
                if ~isa(deps{i}, "types.Parameter")
                    error("All dependencies must be types.Parameter instances.");
                end
            end
        end
        ...
        function [value, expr, records, inferred_unit] = build_from_function(fun_handle, deps)
            ...
            for i = 1:numel(deps)
                ...
                records = types.Parameter.merge_dependency_records(records, deps{i}.dependency_records);
            end
            value = fun_handle(arg_values{:});
            if ~(isnumeric(value) && isscalar(value) && isfinite(value))
                error("Function-based Parameter must evaluate to a finite scalar numeric value.");
            end
            inferred_unit = types.Parameter.infer_dependency_unit(deps);
        end
