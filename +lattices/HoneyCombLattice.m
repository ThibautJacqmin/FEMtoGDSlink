classdef HoneyCombLattice < lattices.Lattice
    ...
    methods
        function obj = HoneyCombLattice(a, nw, nh)
            ...
            obj.hexagonal_lattice = lattices.HexagonalLattice(obj.a, obj.nw, obj.nh);
        ...
        function feature = latticeFromFeature(obj, args)
            ...
            switch key
                case {"a", "sublatticea", "sitea"}
                    feature = obj.hexagonal_lattice.latticeFromFeature(ctx=ctx, seed=seedA, ...
                        layer=layer, a_parameter=args.a_parameter, offset=[0, 0]);
                case {"b", "sublatticeb", "siteb"}
                    feature = obj.hexagonal_lattice.latticeFromFeature(ctx=ctx, seed=seedB, ...
                        layer=layer, a_parameter=args.a_parameter, offset=[0.5, 1/(2*sqrt(3))]);
                case {"ab", "all", "*"}
                    arr_A = obj.hexagonal_lattice.latticeFromFeature(ctx=ctx, seed=seedA, ...
                        layer=layer, a_parameter=args.a_parameter, offset=[0, 0]);
                    arr_B = obj.hexagonal_lattice.latticeFromFeature(ctx=ctx, seed=seedB, ...
                        layer=layer, a_parameter=args.a_parameter, offset=[0.5, 1/(2*sqrt(3))]);
                    feature = ops.Union(ctx, {arr_A, arr_B}, layer=layer);
                case {"centers", "hexagon_centers", "center"}
                    feature = obj.hexagonal_lattice.latticeFromFeature(ctx=ctx, seed=seedCenters, ...
    ...
    methods (Access=private)
        function [ctx, layer, seedA, seedB, seedCenters] = resolve_seeds(~, args)
            ...
            if isempty(seedA) && isempty(seedB) && isempty(seedCenters)
                error("HoneyCombLattice latticeFromFeature requires seed or seedA/seedB/seedCenters.");
            end
