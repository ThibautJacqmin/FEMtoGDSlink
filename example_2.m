...
ctx = GeometryPipeline(enable_comsol=true, enable_gds=true, ...
...
Array2D(array_seed_grid, ncopies_x=p_arr_nx, ncopies_y=p_arr_ny, ...
    delta_x=p_arr_pitch_x*Vertices([1, 0]), ...
    delta_y=p_arr_pitch_y*Vertices([0, 1]), ...
    layer="metal1");
...
[~, ~] = Lattice.createLattice(lattice="Hexagonal", ...
    a=p_lattice_a.value, nw=5, nh=4, seed=seed_hex, ...
